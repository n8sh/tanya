...
interface MessageAuthenticationCode
{
     ...
     *  secret = Secret key.
     */
    void start(ref const Array!ubyte secret) @nogc;
    ...
    if (hasBlockSize!H)
{
     ...
     *  secret = Secret key.
     */
    void start(ref const Array!ubyte secret) pure nothrow @trusted
    {
     ...
     * Returns: The hash of the current message.
     */
    Array!ubyte finish() nothrow @safe
    {
        this.odigest.put(this.idigest.finish());

        return Array!ubyte(odigest.finish());
    }
...
nothrow @nogc unittest
{
    ...
    auto key = Array!ubyte(16, 0x0b);
    auto text = const Array!ubyte(['H', 'i', ' ', 'T', 'h', 'e', 'r', 'e']);
...
private nothrow @nogc unittest
{
    ...
    auto hmac = defaultAllocator.make!(HMAC!MD5);
    { // 2
        auto key = Array!ubyte(['J', 'e', 'f', 'e']);
        auto text = const Array!ubyte(['w', 'h', 'a', 't', ' ', 'd', 'o', ' ', 'y', 'a', ' ',
    ...
    { // 3
        auto key = Array!ubyte(16, 0xaa);
        auto text = const Array!ubyte(50, 0xdd);
    ...
    { // 4
        auto key = Array!ubyte(cast(ubyte[25])
                                [ 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09,
                                  0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f, 0x10, 0x11, 0x12,
                                  0x13, 0x14, 0x15, 0x16, 0x17, 0x18, 0x19, ]);
        auto text = const Array!ubyte(50, 0xcd);
    ...
    { // 5
        auto key = Array!ubyte(16, 0x0c);
        auto text = const Array!ubyte([ 'T', 'e', 's', 't', ' ', 'W', 'i', 't', 'h', ' ',
                                   'T', 'r', 'u', 'n', 'c', 'a', 't', 'i', 'o', 'n' ]);
    ...
    { // 6
        auto key = Array!ubyte(80, 0xaa);
        auto text = const Array!ubyte([ 'T', 'e', 's', 't', ' ', 'U', 's', 'i', 'n', 'g', ' ',
                                   'L', 'a', 'r', 'g', 'e', 'r', ' ', 'T', 'h', 'a', 'n', ' ',
                                   'B', 'l', 'o', 'c', 'k', '-', 'S', 'i', 'z', 'e', ' ',
                                   'K', 'e', 'y', ' ', '-', ' ', 'H', 'a', 's', 'h', ' ',
                                   'K', 'e', 'y', ' ', 'F', 'i', 'r', 's', 't' ]);
